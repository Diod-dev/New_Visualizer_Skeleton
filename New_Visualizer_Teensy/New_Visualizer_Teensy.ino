...
void setup()
{
  ...
  // Test all LEDs
  for (int i = 0; i < NUM_STRIPS * NUM_LEDS_PER_STRIP; i++) leds[i] = CHSV(0, 0, 30);
  FastLED.show();
  delay(1000);
...
void loop()
{
	...
	// This is the function that makes it all work. When testing new patterns or functions, comment out normalOperation() and just put the pattern below.
	normalOperation();

  // Use buttons to change modes
  if (button1.pressed()) {
    MVPattern++;
    if (MVPattern == 5) MVPattern = 0;
  }
  if (button2.pressed()) {
    patternMode++;
    if (patternMode == 5) patternMode = 0;
  }
	
	// * Diagnostic functions if you need to test something *
	//printSpectrum(); // prints the whole frequency spectrum. Useful to see if music is being input correctly.
  ...
  //delay(5); // If the LEDs look glitchy, flip the comment/uncomment on this line.
	FastLED.show();
}
...
void autoMusicVisualizing() {
	...
	if (mixAmount == 255 && mixAmountOld != 255) {
		currentPaletteNumber = random8(gGradientPaletteCount); // generate random number within number of palettes
  ...
  // Save the previous mix amount to mixAmountOld. Useful for tracking its changes
